...
const CLOUD_INITIAL_X = -500;

const FOG_COLOR = 0x1e1e1e;
const FOG_START = CAMERA_Z;
const FOG_END = CAMERA_Z + CLOUD_OUTER_RADIUS * 1.4;
...
function init() {
	...
	const showSelectionSphereMaterial = new THREE.MeshBasicMaterial( {color: 0xF66717, transparent: true, opacity: HELPER_MESH_OPACITY } );
	...
	const shellMaterial = new THREE.MeshBasicMaterial( {color: 0x6698CB, transparent: true, opacity: HELPER_MESH_OPACITY } );
...
function render() {
	...
	if (initialAnimationComplete) {
		...
		const intersect = raycaster.intersectObject( selectionShell );
		...
		lineGroup.children.forEach(function(line) {
			...
			if (intersectionFound) {
				line.geometry.vertices.forEach(function(vertex) {
					...
					selected = selectionSphere.containsPoint(vector);3
				});
			}
		...
		// cloudGroup.rotation.x = time * CLOUD_X_ROTATION_SPEED;
		// cloudGroup.rotation.y = time * CLOUD_Y_ROTATION_SPEED;
		// cloudGroup.rotation.z = time * CLOUD_Z_ROTATION_SPEED;
